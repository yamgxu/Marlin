...
namespace ExtUI {
  ...
  void onConfigurationStoreRead(bool success) {
    ...
    void onMeshUpdate(const int8_t xpos, const int8_t ypos, const_float_t zval) {
      ...
      //SERIAL_ECHOLNPAIR("onMeshUpdate() x:", xpos, " y:", ypos, " z:", zval);//序列回波对（“onMeshUpdate（）x:，xpos，“y:，ypos，”z:，zval”）；
    }

    void onMeshUpdate(const int8_t xpos, const int8_t ypos, const probe_state_t state) {
      // Called to indicate a special condition//被调用以指示特殊情况
      //SERIAL_ECHOLNPAIR("onMeshUpdate() x:", xpos, " y:", ypos, " state:", state);//序列回波对（“onMeshUpdate（）x:，xpos，“y:，ypos，”state:，state”）；
    }
  #endif
